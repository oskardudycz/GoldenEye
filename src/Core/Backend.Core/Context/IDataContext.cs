...
namespace GoldenEye.Backend.Core.Context
{
    public interface IDataContext : IDisposable
    {
        ...
        Task<TEntity> UpdateAsync<TEntity>(TEntity entity, int? version = null) where TEntity : class;

        TEntity Remove<TEntity>(TEntity entity, int? version = null) where TEntity : class;

        Task<TEntity> RemoveAsync<TEntity>(TEntity entity, int? version = null) where TEntity : class;

        TEntity GetById<TEntity>(object id) where TEntity : class, new();

        Task<TEntity> GetByIdAsync<TEntity>(object id) where TEntity : class, new();
