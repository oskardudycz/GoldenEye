CREATE TABLE [dbo].[_Czynności - Struktura, Koniec-Operacja] (
    ...
    CONSTRAINT [PK2_Czynności - Struktura, Koniec-Operacja] PRIMARY KEY NONCLUSTERED ([Id] ASC),
    CONSTRAINT [FK_Czynności - Struktura, Koniec-Operacja_IdArch] FOREIGN KEY ([IdArch]) REFERENCES [dbo].[_Czynności - Struktura, Koniec-Operacja] ([Id]),
    CONSTRAINT [FK_Czynności - Struktura, Koniec-Operacja_IdArchLink] FOREIGN KEY ([IdArchLink]) REFERENCES [dbo].[_Czynności - Struktura, Koniec-Operacja] ([Id])
);


GO
CREATE CLUSTERED INDEX [PK_Czynności - Struktura, Koniec-Operacja]
    ON [dbo].[_Czynności - Struktura, Koniec-Operacja]([Id] ASC);


GO

CREATE TRIGGER [dbo].[WartoscZmiany_Czynności - Struktura, Koniec-Operacja_UPDATE]
   ON  [dbo].[_Czynności - Struktura, Koniec-Operacja] 
   AFTER UPDATE
AS 
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN
		...
		BEGIN
			...
			INSERT INTO [dbo].[_Czynności - Struktura, Koniec-Operacja]
			...
			UPDATE [dbo].[_Czynności - Struktura, Koniec-Operacja]
...
CREATE TRIGGER [dbo].[WartoscZmiany_Czynności - Struktura, Koniec-Operacja_INSERT]
   ON  [dbo].[_Czynności - Struktura, Koniec-Operacja] 
   AFTER INSERT
AS 
BEGIN
	...
	BEGIN
		IF EXISTS(
			...
			FROM inserted AS S1
			  JOIN [dbo].[_Czynności - Struktura, Koniec-Operacja] AS S2
...
END	
GO
DISABLE TRIGGER [dbo].[WartoscZmiany_Czynności - Struktura, Koniec-Operacja_INSERT]
    ON [dbo].[_Czynności - Struktura, Koniec-Operacja];
