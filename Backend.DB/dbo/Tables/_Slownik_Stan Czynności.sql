CREATE TABLE [dbo].[_Slownik_Stan Czynności] (
    ...
    CONSTRAINT [PK2_Slownik_Stan Czynności] PRIMARY KEY NONCLUSTERED ([Id] ASC),
    CONSTRAINT [FK_Slownik_Stan Czynności_IdArch] FOREIGN KEY ([IdArch]) REFERENCES [dbo].[_Slownik_Stan Czynności] ([Id]),
    CONSTRAINT [FK_Slownik_Stan Czynności_IdArchLink] FOREIGN KEY ([IdArchLink]) REFERENCES [dbo].[_Slownik_Stan Czynności] ([Id]),
    CONSTRAINT [FK_Slownik_Stan Czynności_TypId] FOREIGN KEY ([TypId]) REFERENCES [dbo].[Cecha_Typy] ([Id])
);


GO
CREATE CLUSTERED INDEX [PK_Slownik_Stan Czynności]
    ON [dbo].[_Slownik_Stan Czynności]([Id] ASC);


GO
CREATE TRIGGER [dbo].[WartoscZmiany_Slownik_Stan Czynności_INSERT]
				   ON  [dbo].[_Slownik_Stan Czynności] 
				   AFTER INSERT
				AS 
				BEGIN
					...
					BEGIN
						IF EXISTS(
							...
							FROM inserted AS S1
							  JOIN [dbo].[_Slownik_Stan Czynności]  AS S2
						...
						BEGIN						
							...
							UPDATE [dbo].[_Slownik_Stan Czynności] SET 
				...
				END
GO
DISABLE TRIGGER [dbo].[WartoscZmiany_Slownik_Stan Czynności_INSERT]
    ON [dbo].[_Slownik_Stan Czynności];


GO

				CREATE TRIGGER [dbo].[WartoscZmiany_Slownik_Stan Czynności_UPDATE]
				   ON [dbo].[_Slownik_Stan Czynności] 
				   AFTER UPDATE
				AS 
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							INSERT INTO [dbo].[_Slownik_Stan Czynności]
							...
							UPDATE [dbo].[_Slownik_Stan Czynności]
