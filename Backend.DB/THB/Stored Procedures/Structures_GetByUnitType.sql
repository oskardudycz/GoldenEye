...
-- Description:	Pobiera dane z tabeli Struktura_Obiekt dla podanego typu obiektu (Id typu obiektu) będącego korzeniem struktury.

-- XML wejsciowy w postaci:

	--<Request RequestType="Structures_GetByUnitType" UserId="1" AppDate="2012-02-09T14:45:11" GetFullColumnsData="true"
	--	xsi:noNamespaceSchemaLocation="14.6.Request.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
...
-- =============================================
CREATE PROCEDURE [THB].[Structures_GetByUnitType]
(
...
BEGIN
	...
	DECLARE @Query nvarchar(max) = '',
		...
		EXEC [dbo].[ValidateXML] @XSDSchemaName = 'Schema_Structures_GetByUnitType', @XmlData = @XMLDataIn, @Success = @xmlOk OUTPUT, @ERRMSG = @ERRMSG OUTPUT
		...
		BEGIN
			...
			IF @RequestType = 'Structures_GetByUnitType'
			BEGIN
							...
							(
								...
								(
									...
									(
										...
										WHERE tso.TypObiektuIdRoot = ' + CAST(@TypObiektuId AS varchar);
...
---					
				...
				END
				ELSE
					EXEC [THB].[GetErrorMessage] @Nazwa = N'OPERATION_FORBIDDEN', @Grupa = N'PROC_RESULT', @Val1 = @UzytkownikID, @Val2 = N'Structures_GetByUnitType', @Wiadomosc = @ERRMSG OUTPUT 
			END
			ELSE
				EXEC [THB].[GetErrorMessage] @Nazwa = N'WRONG_REQUEST_TYPE', @Grupa = N'PROC_RESULT', @Val1 = N'Structures_GetByUnitType', @Wiadomosc = @ERRMSG OUTPUT 		
	...
	SET @XMLDataOut = '<?xml version="1.0" encoding="utf-8"?><Response ResponseType="Structures_GetByUnitType"' 
