...
-- Wykonuje wpierw algorytm A1 a następnie A2.

-- Przykladowy plik wejsciowy XML:
	--<?xml version="1.0"?>
	--<Request AppDate="2012-02-09T11:45:23" StructureId="23" RequestType="Algorithm_CalculationOfWater" UserId="1" BranchId="1"
...
-- =============================================
CREATE PROCEDURE [THB].[Algorithm_CalculationOfWater]
(
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN	
			...
			IF @RequestType = 'Algorithm_CalculationOfWater'
			BEGIN
				...
				BEGIN					
					...
					BEGIN
						...
						BEGIN
							...
							--jesli podano Id cechy wagowej to proba rozdzielenia wartosci cechy
							IF @Success = 1 AND @IdCechaC3 IS NOT NULL AND @IdCechaC3 > 0
							BEGIN
								PRINT 'Rozdzielanie wartości cechy';
						...
						COMMIT TRAN CALC_WATER
					END			
				END
				ELSE
					EXEC [THB].[GetErrorMessage] @Nazwa = N'OPERATION_FORBIDDEN', @Grupa = N'PROC_RESULT', @Val1 = @UzytkownikID, @Val2 = N'Algorithm_CalculationOfWater', @Wiadomosc = @ERRMSG OUTPUT
			END
			ELSE
				EXEC [THB].[GetErrorMessage] @Nazwa = N'WRONG_REQUEST_TYPE', @Grupa = N'PROC_RESULT', @Val1 = N'Algorithm_CalculationOfWater', @Wiadomosc = @ERRMSG OUTPUT
		END
...
--	SET @ResultString = (SELECT C.value('text()[1]', 'nvarchar(100)') FROM @ResultValue.nodes('/*') AS t(c));
	...
	SET @XMLDataOut = '<?xml version="1.0" encoding="utf-8"?><Response ResponseType="Algorithm_CalculationOfWater"'
