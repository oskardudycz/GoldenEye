...
-- Description:	Usuwa powiazanie - grupy uzytkownikow - uzytkownicy

-- Przyjmuje XML wejsciowy w postaci:

	--<Request RequestType="UserGroups_DeleteAssignedUsers" UserId="1" AppDate="2012-02-09T12:45:34" IsSoftDelete="true"
...
-- Zwraca XML w postaci:
	...
	--<Response ResponseType="UserGroups_DeleteAssignedUsers" AppDate="2012-02-09">
	--	<Result>
	--		<Value>true</Value>
	--	</Result>
	--</Response>
...
-- =============================================
CREATE PROCEDURE [THB].[UserGroups_DeleteAssignedUsers]
(
...
BEGIN
	...
	BEGIN TRY
		...
		CREATE TABLE #DoUsuniecia(IdGrupy int, IdUzytkownika int);
		...
		BEGIN	
			...
			INSERT INTO #DoUsuniecia (IdGrupy, IdUzytkownika)
			SELECT C.value('../@Id', 'int')
			, C.value('./@Id', 'int') 
			FROM @xml_data.nodes('/Request/Ref/Ref') T(C)
			WHERE C.value('../@EntityType', 'nvarchar(30)') = 'UserGroup' AND C.value('./@EntityType', 'nvarchar(30)') = 'User'		
			
			--SELECT @DataProgramu, @RequestType, @UzytkownikID, @UsuwanieMiekkie			
			--SELECT * FROM #DoUsuniecia;

			IF @RequestType = 'UserGroups_DeleteAssignedUsers'
			BEGIN				
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN									
							...
							WHERE GrupaUzytkownikow = @IdGrupy AND Uzytkownik IN (SELECT IdUzytkownika FROM #DoUsuniecia WHERE IdGrupy = @IdGrupy);
						END
						ELSE
						BEGIN
							...
							RealCreatedOn = @DataUsuniecia,
							RealDeletedFrom = @DataUsuniecia
							WHERE GrupaUzytkownikow = @IdGrupy AND Uzytkownik IN (SELECT IdUzytkownika FROM #DoUsuniecia WHERE IdGrupy = @IdGrupy) AND IsValid = 1 AND IsDeleted = 0;
						END
				...
				END
				ELSE
					EXEC [THB].[GetErrorMessage] @Nazwa = N'OPERATION_FORBIDDEN', @Grupa = N'PROC_RESULT', @Val1 = @UzytkownikID, @Val2 = N'UserGroups_DeleteAssignedUsers', @Wiadomosc = @ERRMSG OUTPUT
			END
			ELSE
				EXEC [THB].[GetErrorMessage] @Nazwa = N'WRONG_REQUEST_TYPE', @Grupa = N'PROC_RESULT', @Val1 = N'UserGroups_DeleteAssignedUsers', @Wiadomosc = @ERRMSG OUTPUT
		END
	...
	SET @XMLDataOut = '<?xml version="1.0" encoding="utf-8"?><Response ResponseType="UserGroups_DeleteAssignedUsers"';
