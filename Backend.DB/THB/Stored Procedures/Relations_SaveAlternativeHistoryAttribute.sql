...
-- Przykladowy plik XML wejsciowy:
	--<?xml version="1.0"?>
	--<Request UserId="1" AppDate="2012-09-09T11:23:22" RequestType="Units_SaveAlternativeHistoryAttribute" RelationId="3">
...
-- Przykłądowy plik XML wyjściowy:
	...
	--</Response>
-- =============================================
CREATE PROCEDURE [THB].[Relations_SaveAlternativeHistoryAttribute]
(
...
BEGIN
	...
	BEGIN TRY
		...
		EXEC [dbo].[ValidateXML] @XSDSchemaName = 'Schema_Relations_SaveAlternativeHistoryAttribute', @XmlData = @XMLDataIn, @Success = @xmlOk OUTPUT, @ERRMSG = @ERRMSG OUTPUT
		...
		BEGIN	
			...
			SELECT @IdCechyRelacji = x.value('./@Id','int')
...
--SELECT * FROM #CechyDoZastapienia
			...
			IF @RequestType = 'Relations_SaveAlternativeHistoryAttribute'
			BEGIN 
				...
				BEGIN
					...
					BEGIN
						...
						SET @Query = '
							...
							WHERE c.RelacjaId = ' + CAST(@RelacjaId AS varchar) + ' AND c.CechaID = ' + CAST(@CechaId AS varchar) + ' AND c.IsMainHistFlow = 1
					...
					BEGIN
						...
						SET @Query = '
							...
							SELECT Id, CechaId, ObowiazujeOd, ObowiazujeDo, ValidFrom, ValidTo, IsValid, ISNULL(LastModifiedOn, CreatedOn), ColumnsSet, ValString
							FROM [Relacja_Cecha_Hist] c
							WHERE RelacjaId = ' + CAST(@RelacjaId AS varchar) + ' AND CechaID = ' + CAST(@CechaId AS varchar) + ' AND IsMainHistFlow = 1
...
--SELECT * FROM #PrzedzialyWpisow
					...
					BEGIN TRAN T1_Relations_SaveAlternative							
...
--SELECT @WartoscCechyQuery, @TypWartosciCechy AS TYPWartosci, @WartoscCechy AS WartoscCechy, @IdSlownika AS IdSlownika, @IdElementuSlownika AS IdElementuSlownika
					...
					SET @Query = '
						IF EXISTS (SELECT Id FROM [Relacja_Cecha_Hist]
							WHERE IsMainHistFlow = 0 AND CechaId = ' + CAST(@CechaId AS varchar) + ' AND RelacjaId = ' + CAST(@RelacjaId AS varchar) + ' AND ' + @DateFromColumnName + ' = ' + @DataObowiazywaniaOdStr + ')
						BEGIN
...
--SELECT @CzyJestHistoriaAlternatywnaJuzWBazie AS CzyJestHistoriaAlternatywnaJuzWBazie					
					...
					SET @Query = '
						...
						FROM dbo.[Relacja_Cecha_Hist] 
						WHERE ' + @DateFromColumnName + ' = ' + @DataObowiazywaniaOdStr + ' AND CechaId = ' + CAST(@CechaId AS varchar) + ' AND RelacjaId = ' + CAST(@RelacjaId AS varchar) + '
...
--SELECT @CechaAlternatywnaIstniejeId AS CechaAlternatywnaId_Istniejaca, @DataObowiazywaniaOd AS ObowiazujeOd
					...
					-- jesli znaleziono wpis dla podanej relacji, cechy i wartosc i flaga isMainHistFlow w podanym przedziale to rzucenie bledu
					...
					BEGIN						
						...
						FROM dbo.[Relacja_Cecha_Hist]
						WHERE CechaId = @CechaId AND RelacjaId = @RelacjaId
					...
					BEGIN
						...
						SET @Query = '																					
							UPDATE [Relacja_Cecha_Hist] SET
								...
								LastModifiedBy = ' + CAST(@UzytkownikId AS varchar) + ',
								RealLastModifiedOn = ''' + CONVERT(varchar, @DataModyfikacji, 109) + '''
							WHERE CechaId = ' + CAST(@CechaId AS varchar) + ' AND RelacjaId = ' + CAST(@RelacjaId AS varchar) + ' AND IsAlternativeHistory = 0
								AND ' + @DateFromColumnName + ' = ' + @DataObowiazywaniaOdStr + ';'
						...
						BEGIN
							...
							BEGIN
								...
								SET @Query += '			
										...
										LastModifiedBy = ' + CAST(@UzytkownikId AS varchar) + ',
										RealLastModifiedOn = ''' + CONVERT(varchar, @DataModyfikacji, 109) + '''
									WHERE CechaId = ' + CAST(@CechaId AS varchar) + ' AND RelacjaId = ' + CAST(@RelacjaId AS varchar) + ' AND Id = ' + CAST(@IdGlownegoRekorduZCecha AS varchar)
					...
					COMMIT TRAN T1_Relations_SaveAlternative

				END
				ELSE
					EXEC [THB].[GetErrorMessage] @Nazwa = N'OPERATION_FORBIDDEN', @Grupa = N'PROC_RESULT', @Val1 = @UzytkownikID, @Val2 = N'Relations_SaveAlternativeHistoryAttribute', @Wiadomosc = @ERRMSG OUTPUT
			END
			ELSE
				EXEC [THB].[GetErrorMessage] @Nazwa = N'WRONG_REQUEST_TYPE', @Grupa = N'PROC_RESULT', @Val1 = N'Relations_SaveAlternativeHistoryAttribute', @Wiadomosc = @ERRMSG OUTPUT	
		END
	...
	BEGIN CATCH
		...
		IF @@TRANCOUNT > 0
		BEGIN
			ROLLBACK TRAN T1_Relations_SaveAlternative
		END
	END CATCH

	--przygotowanie XMLa zwrotnego
	SET @XMLDataOut = '<?xml version="1.0" encoding="utf-8"?><Response ResponseType="Relations_SaveAlternativeHistoryAttribute"';
