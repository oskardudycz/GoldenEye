...
-- Description:	Pobiera dane relacji danego typu z tabeli Relacje z uwzglednieniem filrów. Pobierane sa takze obiekty L i P.
--•	filtr
--•	sortowanie
--•	stronicowanie
...
-- XML wejsciowy w postaci:
	...
	--<Request RequestType="Relations_GetOfType" TypeId="3" UserId="1" AppDate="2012-02-09T11:34:08" GetFullColumnsData="true" ExpandNestedValues="true">
...
-- XML wyjsciowy w postaci:
	...
	--<?xml version="1.0" encoding="utf-8"?>
	--<Response ResponseType="Relations_GetOfType" AppDate="2012-02-09Z" xsi:noNamespaceSchemaLocation="2.2.Response.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	--</Response>
-- =============================================
CREATE PROCEDURE [THB].[Relations_GetOfType]
(
...
BEGIN
	...
	DECLARE @Query nvarchar(max) = '',
		...
		CREATE TABLE #IDRelacji (Id int PRIMARY KEY, IdArch int);
		...
		BEGIN
			...
			BEGIN		
				...
				BEGIN		
					...
					EXEC [THB].[PrepareFilters_Relations_GetOfType]
						@XMLDataIn = @XMLDataIn,
...
--SELECT @WhereClause, @OrderByClause, @RozmiarStrony, @NumerStrony		
							...
							(
								...
								(
									...
									(
										...
										FROM [dbo].[Relacje] r3
										WHERE TypRelacji_ID = ' + CAST(@TypRelacjiId AS varchar)									
								...
								(
									...
									(
										...
										(
											...
											WHERE rch3.RelacjaID IN (SELECT DISTINCT ISNULL(IdArch, Id) FROM #IDRelacji) AND rch3.IsMainHistFlow = 1'									
...
--SELECT * FROM #IDRelacjiFinal
--SELECT * FROM #CechyRelacjiId
						...
						JOIN dbo.[Relacje] r ON (r.Id = ch.RelacjaID) -- OR r.IdArch = ch.RelacjaID)
						LEFT OUTER JOIN dbo.[TypRelacji_Cechy] trc ON (trc.Cecha_ID = c.Cecha_ID AND trc.TypRelacji_ID = r.TypRelacji_ID)
						WHERE ch.Id IN (SELECT DISTINCT Id FROM #CechyRelacjiId) AND (trc.ID IN (SELECT Id FROM #TypRelacjiCechy) OR trc.ID IS NULL)'
...
--SELECT * FROM #CechyRelacji	
						...
						SET @Query = N'INSERT INTO #CechyTypuRelacji(CechaId, TypRelacjiId, [Priority], UIOrder)
							...
							WHERE TypRelacji_ID IN (SELECT DISTINCT TypRelacji_ID FROM Relacje WHERE Id IN (SELECT ID FROM #IDRelacji))' --IdArch IS NULL AND IsValid = 1 AND IsDeleted = 0 AND
					...
					IF Cursor_Status('local','cur_Relations_GetOfType') > 0 
					BEGIN
						 CLOSE cur_Relations_GetOfType
						 DEALLOCATE cur_Relations_GetOfType
					END
				
					DECLARE cur_Relations_GetOfType CURSOR LOCAL FOR 
						SELECT Id, ISNULL(IdArch, Id) FROM #IDRelacjiFinal ORDER BY Rn  --distinct
					OPEN cur_Relations_GetOfType
					FETCH NEXT FROM cur_Relations_GetOfType INTO @IdRelacji, @IdArchRelacji
					...
					BEGIN
						...
						BEGIN
							...
							BEGIN
								...
								DECLARE cur2 CURSOR LOCAL FOR 
									SELECT Id, SparceValue, ValString, CzySlownik, TypCechyId, CechaId, IsAlternativeHistory FROM #CechyRelacji WHERE RelacjaId = @IdArchRelacji ORDER BY Id -- (RelacjaId = @IdRelacji OR RelacjaId = @IdArchRelacji) ORDER BY Id
								OPEN cur2
								FETCH NEXT FROM cur2 INTO @CechaRelacjiId, @XmlSparse, @WartoscString, @CzySlownik, @CechaTypId, @CechaId, @CechaHasAlternativeHistory 
								WHILE @@FETCH_STATUS = 0
								BEGIN
												...
												BEGIN
													...
													SET @query += ' , (SELECT TOP 1 ISNULL(c2.[ZmianaOd], c2.[CreatedOn]) AS "@ChangeFrom"
																	...
																	FROM #CechyRelacji c2
																		WHERE c2.[Id] <> ' + CAST(@CechaRelacjiId AS varchar) + ' AND c2.RelacjaId = ' + CAST(@IdArchRelacji AS varchar) --+ ' OR c2.RelacjaId = ' + CAST(@IdArchRelacji AS varchar) + ')'
												...
												BEGIN
													...
													SET @Query += ', ( SELECT ''' + [THB].[PrepareXMLValue](@CechaWartosc) + ''' AS "@Value"
																,( SELECT TOP 1 ISNULL(c2.[ZmianaOd], c2.[CreatedOn]) AS "@ChangeFrom"
																	...
																	,ISNULL(c2.[IsMainHistFlow], 0) AS "@IsMainHistFlow"
																	FROM #CechyRelacji c2
																	WHERE c2.[Id] <> ' + CAST(@CechaRelacjiId AS varchar) + ' AND c2.RelacjaId = ' + CAST(@IdArchRelacji AS varchar) --@IdRelacji
																		+ ' AND c.[CechaId] = c2.[CechaId]
																	FOR XML PATH(''History''), TYPE)
									...
									SET @query += '	
										FROM #CechyRelacji c
										WHERE c.[RelacjaId] = ' + CAST(@IdArchRelacji AS varchar) + ' AND Id = ' + CAST(@CechaRelacjiId AS varchar) + ' --@IdRelacji
						...
						BEGIN
							...
							BEGIN
								...
								--pobieranie danych podwezlow, cech obiektu
								DECLARE cur2 CURSOR LOCAL FOR 
									SELECT Id, SparceValue, ValString, CzySlownik, TypCechyId, CechaId, IsAlternativeHistory  FROM #CechyRelacji WHERE RelacjaId = @IdArchRelacji ORDER BY Id -- ORDER BY Id
								OPEN cur2
								FETCH NEXT FROM cur2 INTO @CechaRelacjiId, @XmlSparse, @WartoscString, @CzySlownik, @CechaTypId, @CechaId, @CechaHasAlternativeHistory
								WHILE @@FETCH_STATUS = 0
								BEGIN
									...
									BEGIN
										...
										BEGIN									
											...
											BEGIN
												...
												IF @CechaHasAlternativeHistory IS NULL OR @CechaHasAlternativeHistory = 0 
												BEGIN
													SET @Query += ', ' + CAST(@CechaWartosc AS varchar) + ' AS "ValDictionary/@ElementId" 
															, ' + CAST(@CechaTypId AS varchar) + ' AS "ValDictionary/@Id"'
												...
												BEGIN
													...
													SET @Query += ', (SELECT TOP 1 ISNULL(c2.[ZmianaOd], c2.[CreatedOn]) AS "@ChangeFrom"
																	...
																	,ISNULL(c2.[IsMainHistFlow], 0) AS "@IsMainHistFlow"
																	FROM #CechyRelacji c2
																		WHERE c2.[Id] <> ' + CAST(@CechaRelacjiId AS varchar) + ' AND c2.RelacjaId = ' + CAST(@IdArchRelacji AS varchar)
																		 + ' AND c.[CechaId] = c2.[CechaId]
																	FOR XML PATH(''History''), TYPE)
																)
																FOR XML PATH(''ValDictionary''), TYPE)'							
									...
									SET @Query += '	
										FROM #CechyRelacji c
										WHERE c.[RelacjaId] = ' + CAST(@IdArchRelacji AS varchar) + ' AND Id = ' + CAST(@CechaRelacjiId AS varchar) + ' 
										FOR XML PATH(''Attribute''), TYPE
										)'
						...
						BEGIN
							SET @query += '
								, (SELECT 0 AS "@Id"
										...
										WHERE bez.TypRelacjiId = r.[TypRelacji_ID] AND bez.[CechaID] NOT IN (SELECT CechaId FROM #CechyRelacji c WHERE c.[RelacjaId] = ' + CAST(@IdArchRelacji AS varchar) + ') --@IdRelacji
										FOR XML PATH(''Attribute''), TYPE
										)'
						END
					...
					BEGIN
						...
						SET @query = 'SET @IloscStron = (SELECT COUNT(Id) FROM #IDRelacji)'
				...
				END
				ELSE
					EXEC [THB].[GetErrorMessage] @Nazwa = N'OPERATION_FORBIDDEN', @Grupa = N'PROC_RESULT', @Val1 = @UzytkownikID, @Val2 = N'Relations_GetOfType', @Wiadomosc = @ERRMSG OUTPUT 
			END
			ELSE
				EXEC [THB].[GetErrorMessage] @Nazwa = N'WRONG_REQUEST_TYPE', @Grupa = N'PROC_RESULT', @Val1 = N'Relations_GetOfType', @Wiadomosc = @ERRMSG OUTPUT
		END
		
	--przygotowanie XMLa zwrotnego
	SET @XMLDataOut = '<?xml version="1.0" encoding="utf-8"?><Response ResponseType="Relations_GetOfType"';
