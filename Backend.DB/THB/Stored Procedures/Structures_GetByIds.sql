...
-- XML wejsciowy w postaci:
	...
	--<Request RequestType="Structures_GetByIds" UserId="1" AppDate="2012-02-09T11:54:34" GetFullColumnsData="true">
...
-- =============================================
CREATE PROCEDURE [THB].[Structures_GetByIds]
(
...
BEGIN
	...
	DECLARE @query nvarchar(max) = '',
		...
		BEGIN
			...
			IF @RequestType = 'Structures_GetByIds'
			BEGIN		
				...
				END
				ELSE
					EXEC [THB].[GetErrorMessage] @Nazwa = N'OPERATION_FORBIDDEN', @Grupa = N'PROC_RESULT', @Val1 = @UzytkownikID, @Val2 = N'Structures_GetByIds', @Wiadomosc = @ERRMSG OUTPUT 
			END
			ELSE
				EXEC [THB].[GetErrorMessage] @Nazwa = N'WRONG_REQUEST_TYPE', @Grupa = N'PROC_RESULT', @Val1 = N'Structures_GetByIds', @Wiadomosc = @ERRMSG OUTPUT
			END TRY
			BEGIN CATCH
				SET @ERRMSG = @@ERROR;
				SET @ERRMSG += ' ';
	...
	SET @XMLDataOut = '<?xml version="1.0" encoding="utf-8"?><Response ResponseType="Structures_GetByIds"'
